...
public:
    ...
    void start()
    {
        ...
        chat->sendToPlayer1(Protocol::build(TAG_START,
                                            std::to_string(chat->getId2()) + "|w|" + boardState));

        chat->sendToPlayer2(Protocol::build(TAG_START,
                                            std::to_string(chat->getId1()) + "|b|" + boardState));

        logs->info("GameRoom (Hash Map):- game room [" +
                   std::to_string(roomId) + "] started");
    }
    ...
    GameStatus getStatus() { return game->getStatus(); }
    bool isFinished() { return game->isFinished(); }
    ChatSession *getChat() { return chat; }
    GameSession *getGame() { return game; }
